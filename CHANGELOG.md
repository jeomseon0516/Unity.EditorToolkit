...
- `com.unity.ugui` 의존성 핀이 `1.0.0`(Unity 2022 시절 값)으로 남아 있어, Unity 6000.6 소비
  프로젝트에서 UPM이 존재하지 않는 버전을 요청하고 편집기 기본본으로 fallback하며 콘솔 경고를
  남기던 문제를 고쳤습니다. `1.0.0` → `2.0.0`(6000.6.0f1 동봉 버전, `Unity.TextMeshPro` 어셈블리
  포함). 0.7.2의 sibling 핀 갱신에서 누락된 `com.unity.*` 핀입니다. 코드 변경 없음.
